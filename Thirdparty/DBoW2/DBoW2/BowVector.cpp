...
void BowVector::addWeight(WordId id, WordValue v)//这个函数的作用是：给词袋向量 BowVector 中编号为 id 的视觉单词增加权重 v
{
  BowVector::iterator vit = this->lower_bound(id);//在有序容器中找到第一个键值 >= id 的位置。这个位置既能帮助判断 id 是否已经存在，也能作为高效插入的位置提示。
  ...
  {
    this->insert(vit, BowVector::value_type(id, v));//该单词还没出现，就插入新的键值对
  }
}
