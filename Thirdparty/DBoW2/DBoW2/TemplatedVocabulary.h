...
template<class TDescriptor, class F>
WordId TemplatedVocabulary<TDescriptor, F>::transform//：把一个特征描述子 feature 转换成对应的视觉单词编号 WordId
...
template<class TDescriptor, class F> 
void TemplatedVocabulary<TDescriptor,F>::transform(
  const std::vector<TDescriptor>& features,//一张图像提取出的全部特征描述子
  BowVector &v,//图像的词袋向量
  FeatureVector &fv,//特征索引表（某个词典树节点下有哪些原始特征）
  int levelsup//指定 FeatureVector 记录词典树中离叶子节点向上几层的节点。它用于后续特征匹配时缩小候选范围；常见值是 0，即记录叶子节点本身。
  ) const

{
  v.clear();//每次重新计算前，先清空传入的输出容器，
  fv.clear();
  ...
  // normalize 
  LNorm norm;
  //ustNormalize(norm)： 返回 true：最终必须归一化 v，并把所需范数写入 norm。
  //                     返回 false：当前评分方式不需要显式归一化。
  bool must = m_scoring_object->mustNormalize(norm);//这里确定最后是否需要归一化 BoW 向量
  
  typename vector<TDescriptor>::const_iterator fit;
  
  if(m_weighting == TF || m_weighting == TF_IDF)// 如果采用：TF：词频（Term Frequency）或TF_IDF：词频 × 逆文档频率。则同一个视觉单词在图像中出现多次时，要累计它的权重。
  {
    ...
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
    {
      ...
      transform(*fit, id, w, &nid, levelsup);//对当前描述子做词典树量化
