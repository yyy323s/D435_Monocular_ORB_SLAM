...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        ~GeometricCamera() {}
        //这是一个“纯虚函数”，基类自己不提供实现，派生类必须实现。
        //可以理解成：“所有相机都必须会 project，但每种相机怎么 project，由具体相机模型自己定义。”
        ...
        virtual Eigen::Vector3f unprojectEig(const cv::Point2f &p2D) = 0;//反投影
        virtual cv::Point3f unproject(const cv::Point2f &p2D) = 0;

        virtual Eigen::Matrix<double,2,3> projectJac(const Eigen::Vector3d& v3D) = 0;//二维向量对三维向量求导
        ...
        virtual bool ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2, const std::vector<int> &vMatches12,
                                             Sophus::SE3f &T21, std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated) = 0;//双视图重建

        virtual cv::Mat toK() = 0;//得到相机内参
        virtual Eigen::Matrix3f toK_() = 0;

        virtual bool epipolarConstrain(GeometricCamera* otherCamera, const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, const Eigen::Matrix3f& R12, const Eigen::Vector3f& t12, const float sigmaLevel, const float unc) = 0;//极线约束
