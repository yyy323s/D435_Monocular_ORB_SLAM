...
namespace ORB_SLAM3 {
    class Pinhole : public GeometricCamera {//子类
    ...
    public:
        ...
        Pinhole(const std::vector<float> _vParameters) : GeometricCamera(_vParameters), tvr(nullptr) {
            assert(mvParameters.size() == 4);//必须是fx,fy,cx,cy
            mnId=nNextId++;
            mnType = CAM_PINHOLE;
        }
        ...
        float uncertainty2(const Eigen::Matrix<double,2,1> &p2D);//这个函数用于给一个二维图像点 p2D 返回它的“不确定性平方”：
