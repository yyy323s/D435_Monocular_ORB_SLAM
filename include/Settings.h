...
namespace ORB_SLAM3 {
    ...
    private:
        ...
        void readCamera1(cv::FileStorage& fSettings);//2.从ymal文件读取相机参数
        void readCamera2(cv::FileStorage& fSettings);
        void readImageInfo(cv::FileStorage& fSettings);//3.读取图像信息
        void readIMU(cv::FileStorage& fSettings);
        void readRGBD(cv::FileStorage& fSettings);
        void readORB(cv::FileStorage& fSettings);//4.读取ORB提取参数
        void readViewer(cv::FileStorage& fSettings);//6.略读
        void readLoadAndSave(cv::FileStorage& fSettings);//7.略读
        void readOtherParameters(cv::FileStorage& fSettings);//5.
