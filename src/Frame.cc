     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();//相邻层缩放倍率
    mfLogScaleFactor = log(mfScaleFactor);//log
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();//每层相对于原图的缩放倍率
    mvInvScaleFactors = mpORBextractorLeft->GetInverseScaleFactors();//上一个的倒数
    mvLevelSigma2 = mpORBextractorLeft->GetScaleSigmaSquares();
    mvInvLevelSigma2 = mpORBextractorLeft->GetInverseScaleSigmaSquares();
    ...
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));//Frame每个特征关联Mappoint的容器

    mmProjectPoints.clear();//当前帧中，哪些地图点被投影到图像内，以及其投影相关信息。
    mmMatchedInImage.clear();//当前帧中已经与地图点成功匹配的图像特征

    mvbOutlier = vector<bool>(N,false);

    // 这仅在第一帧时（或校准更改后）进行。//下面这些成员变量都是静态成员变量
    if(mbInitialComputations)//mbInitialComputations 也是一个静态标志位。初始状态通常为true
    {
        ComputeImageBounds(imGray);
...
void Frame::AssignFeaturesToGrid()//一帧中的特征点按二维网格位置分配到 mGrid
{
    ...
    for(unsigned int i=0; i<FRAME_GRID_COLS;i++)
        for (unsigned int j=0; j<FRAME_GRID_ROWS;j++){
            ...
            if(Nleft != -1){//Nleft=-1表示单目或Rgbd相机，如果是双目相机那么Nleft=左图特征数
    ...
    for(int i=0;i<N;i++)
    {
        const cv::KeyPoint &kp = (Nleft == -1) ? mvKeysUn[i]
                                                 : (i < Nleft) ? mvKeys[i]//双目：前Nleft个属于左图
                                                                 : mvKeysRight[i - Nleft];

        int nGridPosX, nGridPosY;
        if(PosInGrid(kp,nGridPosX,nGridPosY)){//如果关键点能被分配到某个网格
            if(Nleft == -1 || i < Nleft)
...
{
    vector<int> vLapping = {x0,x1};//左右图像的重叠边界，单目不用
    if(flag==0)
        //ORBextractor类已重载（），使得它可以像函数一样被调用
...
Eigen::Vector3f Frame::inRefCoordinates(Eigen::Vector3f pCw)
{
    ...
    //x, y：搜索区域的中心坐标。
    //r：搜索半径，不过这里实际搜索的是正方形，不是圆形。
    //minLevel：允许的最小图像金字塔层级。
    //maxLevel：允许的最大图像金字塔层级；负数通常表示不限制。
    //bRight：false：搜索左图特征；
    //true：搜索右图特征。

vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel, const bool bRight) const
{
    vector<size_t> vIndices;//用来保存找到的特征点索引
    vIndices.reserve(N);

    float factorX = r;//设置横向和纵向搜索范围
    float factorY = r;

    //mfGridElementWidthInv=1.0f / gridWidth（网格宽度的倒数）
    //查询区域最左边的像素坐标：x - factorX。网格列 = (像素x - mnMinX) / gridWidth
    //减去图像有效区域的左边界：x - factorX - mnMinX。
    //转换成网格坐标：(x - factorX - mnMinX) * mfGridElementWidthInv，使用 floor() 向下取整，保证左边可能相交的网格不会遗漏。
    const int nMinCellX = max(0,(int)floor((x-mnMinX-factorX)*mfGridElementWidthInv));
    if(nMinCellX>=FRAME_GRID_COLS)//如果最左边的网格都已经超过网格最后一列，说明整个搜索区域都位于图像右侧，不可能找到特征点，直接返回空数组。
    {
        return vIndices;
    }
    ...
    const bool bCheckLevels = (minLevel>0) || (maxLevel>=0);//是否检查金字塔层级

    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)//遍历覆盖的网格
        {   //选择左图或右图网格，mGrid[ix][iy]：左图这个网格中的特征索引，mGridRight[ix][iy]：右图这个网格中的特征索引
            const vector<size_t> vCell = (!bRight) ? mGrid[ix][iy] : mGridRight[ix][iy];
            if(vCell.empty())
                continue;

            for(size_t j=0, jend=vCell.size(); j<jend; j++)//遍历网格中的特征点
            {   //Nleft == -1：没有采用左右特征分开保存的模式，使用去畸变后的特征 mvKeysUn
                //有左右图并且 bRight == false：使用左图特征 mvKeys
                //有左右图并且 bRight == true：使用右图特征 mvKeysRight
                const cv::KeyPoint &kpUn = (Nleft == -1) ? mvKeysUn[vCell[j]]
                                                         : (!bRight) ? mvKeys[vCell[j]]
                                                                     : mvKeysRight[vCell[j]];
                if(bCheckLevels)//检查金字塔层级
                {
                ...
                //计算特征点与查询中心之间的横向和纵向距离
                const float distx = kpUn.pt.x-x;
                const float disty = kpUn.pt.y-y;

                if(fabs(distx)<factorX && fabs(disty)<factorY)//小于半边长r
...
bool Frame::PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY)//计算关键点在哪个网格
{
...
void Frame::UndistortKeyPoints()//关键点消畸变
{
    ...
    // Undistort points
    mat=mat.reshape(2);//表示把 mat 改为 每个元素有 2 个通道 的矩阵。原来 mat 是：N × 2，单通道，每行是：[x, y]。调用后可视为：N × 1，双通道 CV_32FC2，每行是一个二维点：[(x, y)]
...
bool Frame::isInFrustumChecks(MapPoint *pMP, float viewingCosLimit, bool bRight) {//判断一个三维地图点 pMP，从当前帧相机的视角看，是否处于可投影、可观测、值得尝试匹配的范围内。
    // 3D in absolute coordinates
    Eigen::Vector3f P = pMP->GetWorldPos();
    ...
    // Check distance is in the scale invariance region（在尺度不变区域内） of the MapPoint
    const float maxDistance = pMP->GetMaxDistanceInvariance();//取出可观测距离范围
    const float minDistance = pMP->GetMinDistanceInvariance();
    const Eigen::Vector3f PO = P - twc;//计算从当前相机中心 twc 指向地图点位置 P 的三维向量
    const float dist = PO.norm();
    ...
    // Check viewing angle
    Eigen::Vector3f Pn = pMP->GetNormal();//Pn 是地图点 pMP 的平均观测方向，是根据这个地图点历史上被多个相机观测时的方向，统计出来的一个单位向量。

    const float viewCos = PO.dot(Pn) / dist;

    if(viewCos<viewingCosLimit)//cos越大越好，即夹角度数越小越好
        return false;

    //这段是在：地图点通过前面的可见性检查后，预测它在当前图像应出现在哪个 ORB 金字塔层，并把该点在当前帧的投影信息缓存起来，供后面的投影匹配使用。
    const int nPredictedLevel = pMP->PredictScale(dist,this);//预测该点最可能位于金字塔哪一层
    //距离更近 → 图像中点更大 → 往较低分辨率的金字塔层搜索
    //距离更远 → 图像中点更小 → 往较高分辨率的金字塔层搜索
    if(bRight){
