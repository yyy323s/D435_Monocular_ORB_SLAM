...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    bImu(pMap->isImuInitialized()),//记录当前地图的 IMU 是否已经完成惯性初始化。
    mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp),// 该关键帧来自哪个普通帧、该帧采集时间
    mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),//关键帧特征网格的列数和行数
    mfGridElementWidthInv(F.mfGridElementWidthInv), mfGridElementHeightInv(F.mfGridElementHeightInv),//网格参数宽高的逆（像素坐标转换到网格坐标的比例）
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0),//跟踪时的帧标记；地图点融合时的关键帧标记
    mnBALocalForKF(0),//局部 BA 中是否已加入待优化集合
    mnBAFixedForKF(0), //局部 BA 中是否为固定关键帧
    mnBALocalForMerge(0),//地图合并时局部 BA 的标记
    mnBAGlobalForKF(0),//全局BA标记
    mnLoopQuery(0),//回环检测查询 ID
    mnLoopWords(0),//回环检测中共享视觉单词的统计
    mnRelocQuery(0),//重定位查询 ID
    mnRelocWords(0),//重定位中共享单词统计
    mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0),//通用地点识别模块的查询标记、共享单词数和候选得分。
    fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy),//相机内参
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth),//mbf：基线*fx;mb:基线；深度阈值
    N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn),//关键帧的特征总数；原始关键点；去畸变后的关键点；
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
{
    ...
    if(F.Nleft != -1)  mGridRight.resize(mnGridCols);//双目
...
void KeyFrame::AddConnection(KeyFrame *pKF, const int &weight)//给当前关键帧建立或更新一条“共视连接（covisibility connection）”。
{//两个关键帧观测到了足够多相同的 MapPoint，因此认为它们在地图中具有较强关联。//pKF要连接的另一个关键帧。weight：两关键帧共享的地图点数量，通常就是共视边的权重。
    {
        unique_lock<mutex> lock(mMutexConnections);//锁住当前关键帧的连接数据，避免局部建图、回环、关键帧删除等线程同时修改共视图。
    ...
    UpdateBestCovisibles();//更新当前关键帧的“最佳共视关键帧”列表。
}

void KeyFrame::UpdateBestCovisibles()
{
    unique_lock<mutex> lock(mMutexConnections);//锁住的是当前 KeyFrame 的“连接关系数据”，通常包括：mConnectedKeyFrameWeights，mvpOrderedConnectedKeyFrames，mvOrderedWeights
    vector<pair<int,KeyFrame*> > vPairs;//(weight, KeyFrame*)
    vPairs.reserve(mConnectedKeyFrameWeights.size());//提前分配空间
    for(map<KeyFrame*,int>::iterator mit=mConnectedKeyFrameWeights.begin(), mend=mConnectedKeyFrameWeights.end(); mit!=mend; mit++)
       vPairs.push_back(make_pair(mit->second,mit->first));

    sort(vPairs.begin(),vPairs.end());//从小到大排序，先比较 pair.first
    list<KeyFrame*> lKFs;
    list<int> lWs;
    for(size_t i=0, iend=vPairs.size(); i<iend;i++)
    {
        ...
        {
            lKFs.push_front(vPairs[i].second);//上面sort是从小到大排序，这里利用链表前插O(1)复杂度，不断在头部插入。最终得到从大到小排序。
...
void KeyFrame::AddMapPoint(MapPoint *pMP, const size_t &idx)//idx:此关键帧上对应此3维点的特征索引
{
    unique_lock<mutex> lock(mMutexFeatures);
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    set<MapPoint*> s;//set元素唯一，重复插入同一个指针不会产生第二份
    for(size_t i=0, iend=mvpMapPoints.size(); i<iend; i++)
    {
        if(!mvpMapPoints[i])
            continue;
...
int KeyFrame::TrackedMapPoints(const int &minObs)//返回此KeyFrame跟踪的Mappoint点的数量
{
    unique_lock<mutex> lock(mMutexFeatures);
...
//统计当前关键帧与其他关键帧共享了多少个地图点，建立/更新共视图连接，并在第一次建立连接时设置生成树父节点。
//共视图中的边权重可以理解为：当前 KeyFrame 与另一 KeyFrame 共同观测到的 MapPoint 数量
void KeyFrame::UpdateConnections(bool upParent)
{
    map<KeyFrame*,int> KFcounter;//（其他 KeyFrame* ，与当前关键帧共享的 MapPoint 数量）

    vector<MapPoint*> vpMP;//当前关键帧关联的所有 MapPoint

    {
        unique_lock<mutex> lockMPs(mMutexFeatures);
        vpMP = mvpMapPoints;//取出当前KeyFrame的MapPoint
    }
    ...
    {//遍历每一个MapPoint
        ...
        if(!pMP)
            continue;//跳过空指针，即当前特征还没有关联地图点。

        if(pMP->isBad())
            continue;//跳过已经被删除、融合替换或标记失效的地图点。

        map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();//取出它的观测

        for(map<KeyFrame*,tuple<int,int>>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            if(mit->first->mnId==mnId || mit->first->isBad() || mit->first->GetMap() != mpMap)
                continue;//跳过当前关键帧自己；已失效的关键帧；不属于当前地图的关键帧
            KFcounter[mit->first]++;//如果这个地图点也被另一个有效关键帧观测到，就将该关键帧（不是此关键帧）的共享点计数加一。
            //此计数就是共视权重的来源
        }
    }

    // This should not happen
    if(KFcounter.empty())//此关键帧没有共视关键帧
        return;

    //If the counter is greater than threshold add connection
    //In case no keyframe counter is over threshold add the one with maximum counter
    int nmax=0;//当前发现的最大共视权重
    KeyFrame* pKFmax=NULL;//共视最强的关键帧
    int th = 15;//建立强共视连接的阈值

    vector<pair<int,KeyFrame*> > vPairs;//临时保存满足阈值的连接
    vPairs.reserve(KFcounter.size());
    //upParent == false → 输出共视连接统计信息
    //upParent == true  → 不输出
    if(!upParent)
        cout << "UPDATE_CONN: current KF " << mnId << endl;
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)//遍历每个候选共视关键帧及其权重
    {
        if(!upParent)
            cout << "  UPDATE_CONN: KF " << mit->first->mnId << " ; num matches: " << mit->second << endl;
        if(mit->second>nmax)//持续寻找共享点数最多的关键帧。
        {
            nmax=mit->second;
            pKFmax=mit->first;
        }
        if(mit->second>=th)//如果共享地图点数达到阈值 15
        {   //将该连接存入 vPairs，待会排序；
            //在对方关键帧中也添加一条反向连接。
            vPairs.push_back(make_pair(mit->second,mit->first));
    ...
    {
        lKFs.push_front(vPairs[i].second);//从大到小
        lWs.push_front(vPairs[i].first);
    }

    {
        ...
        mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end());//从大到小排序的共视关键帧
        mvOrderedWeights = vector<int>(lWs.begin(), lWs.end());


        if(mbFirstConnection && mnId!=mpMap->GetInitKFid())//当前关键帧是第一次建立连接；它不是地图初始化的第一个关键帧
        {
