...
void Map::AddKeyFrame(KeyFrame *pKF)
{
    ...
    if(mspKeyFrames.empty()){//如果之前没有KeyFrame加入到此Map
    ...
    if(pKF->mnId>mnMaxKFid)//更新最大KeyFrameid
    {
        mnMaxKFid=pKF->mnId;
    }
    if(pKF->mnId<mpKFlowerID->mnId)//更新最小KeyFrameid
    {
        mpKFlowerID = pKF;
    }
}
...
void Map::SetReferenceMapPoints(const vector<MapPoint *> &vpMPs)//参考地图点设置
{
    unique_lock<mutex> lock(mMutexMap);
...
void Map::InformNewBigChange()
{
    unique_lock<mutex> lock(mMutexMap);
    mnBigChangeIdx++;
    //mnBigChangeIdx   ：重大地图变更版本号
    //mnMapChange      ：一般地图变更版本号
}
...
vector<MapPoint*> Map::GetAllMapPoints()//Map里不仅有Keyframe还有MapPoint
{
    unique_lock<mutex> lock(mMutexMap);
...
long unsigned int Map::GetId()//Map有ID
{
    return mnId;
}
long unsigned int Map::GetInitKFid()//获取初始KF的id
{
    unique_lock<mutex> lock(mMutexMap);
    return mnInitKFid;
}
...
void Map::IncreaseChangeIndex()
{
    unique_lock<mutex> lock(mMutexMap);
    mnMapChange++;
    //mnMapChange = 地图已发生多少轮重要变化。地图发生重要更新时，让版本号加一
    //它通常用于让不同线程知道：自己上次使用地图后，地图是否已被其他线程修改。
}
