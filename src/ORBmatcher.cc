...
namespace ORB_SLAM3
{
    ...
    {
        int nmatches=0;//匹配的数量
        vnMatches12 = vector<int>(F1.mvKeysUn.size(),-1);//F1下特征点对应的F2下标

        vector<int> rotHist[HISTO_LENGTH];//相当于创建了一个长度为 30 的数组，并且数组的每个元素都是一个 vector<int>，每个vector里保存的整数是：属于这个旋转方向组的 F1 特征索引 i1
        for(int i=0;i<HISTO_LENGTH;i++)
            rotHist[i].reserve(500);//给每一个 vector 预留能存放约 500 个整数的内存
        const float factor =static_cast<float>(HISTO_LENGTH) / 360.0f;//factor 用来把角度差 rot 转换成直方图编号

        vector<int> vMatchedDistance(F2.mvKeysUn.size(),INT_MAX);//当前匹配的汉明距离，INT_MAX表示int能表示的最大值。每个 F2 特征目前都还没有匹配，所以暂时把匹配距离看成“无穷大”。
        vector<int> vnMatches21(F2.mvKeysUn.size(),-1);//双向关系

        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            cv::KeyPoint kp1 = F1.mvKeysUn[i1];
            int level1 = kp1.octave;
            if(level1>0)//初始化阶段只使用图像金字塔第 0 层的特征，第 0 层保留了更多原始图像细节，定位也更精确
                continue;

            //vbPrevMatched[i1].x在Tracking初始化的时候传入此函数，预测位置的横坐标
            vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y, windowSize,level1,level1);

            if(vIndices2.empty())//F2对应区域没有特征点
                continue;

            cv::Mat d1 = F1.mDescriptors.row(i1);

            int bestDist = INT_MAX;//F2上最近候选点与i1之间的描述子距离
            int bestDist2 = INT_MAX;//次近
            int bestIdx2 = -1;//最近点的索引

            for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)//遍历F2区域内所有候选点
            {
                size_t i2 = *vit;//取出对应候选点的索引

                cv::Mat d2 = F2.mDescriptors.row(i2);

                int dist = DescriptorDistance(d1,d2);
                //vMatchedDistance[i2]存着之前跟i2匹配上的F1上点与i2的距离
                //举个例子：假如之前F1[3] → F2[8]，汉明距离 = 35
                //现在又发现：F1[11] → F2[8]汉明距离 = 20，显然F1[11]更接近F2[8]，i1=11
                //if（35<20）显然不成立，继续向下执行
                if(vMatchedDistance[i2]<=dist)//跳过已经被更好匹配占用的点
                    continue;

                if(dist<bestDist)
                {
                    bestDist2=bestDist;
                    bestDist=dist;//这次为最近距离
                    bestIdx2=i2;
                }
                else if(dist<bestDist2)
                {
            ...
            if(bestDist<=TH_LOW)//最佳距离必须足够小
            {
                if(bestDist<(float)bestDist2*mfNNratio)//最近邻比值检查
                {
                    if(vnMatches21[bestIdx2]>=0)//vnMatches21[8] = 3，F2的8之前被F1的3占了
                    {
                        vnMatches12[vnMatches21[bestIdx2]]=-1;//vnMatches12[3] = -1;于是撤销旧匹配
                        nmatches--;
                    }
                    vnMatches12[i1]=bestIdx2;//vnMatches12[11] = 8;
                    vnMatches21[bestIdx2]=i1;//vnMatches21[8] = 11;
                    vMatchedDistance[bestIdx2]=bestDist;//vMatchedDistance[8] = 20;
                    nmatches++;

                    if(mbCheckOrientation)//检查方向
                    {
                        float rot = F1.mvKeysUn[i1].angle-F2.mvKeysUn[bestIdx2].angle;
                        if(rot<0.0)
                            rot+=360.0f;//保证方向差落在：[0°, 360°)
                        int bin = static_cast<int>(std::round(rot * factor));//round四舍五入
                        if(bin==HISTO_LENGTH)//bin接近30,代表rot接近360度，把它与0度划到同一个分组
                            bin=0;
                        assert(bin>=0 && bin<HISTO_LENGTH);
                        rotHist[bin].push_back(i1);//如果rotHist[1].size() == 4，表示有 4 组匹配具有这个范围的方向差。
                    }
                }
            }
        ...
        if(mbCheckOrientation)
        {
            ...
            ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);//找出包含匹配最多的三个方向组

            for(int i=0; i<HISTO_LENGTH; i++)
            {
                if(i==ind1 || i==ind2 || i==ind3)//i等于前三多里面任意一个
                    continue;//保留这些桶里的匹配
                for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
                {
                    int idx1 = rotHist[i][j];//取出不是主要方向桶里的F1下标
                    if(vnMatches12[idx1]>=0)//删除这些下标对应的匹配
                    {
                        vnMatches12[idx1]=-1;
                        nmatches--;
                    }
...
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
    ...
    {
        const int *pa = a.ptr<int32_t>();//256的前32的指针
        const int *pb = b.ptr<int32_t>();

        int dist=0;//保存累计汉明距离

        for(int i=0; i<8; i++, pa++, pb++)//循环8次每次处理32位
        {
            unsigned  int v = *pa ^ *pb;//按位异或，0两个描述子在这个位置相同，1两个描述子在这个位置不同
            //统计v中有多少1
            // 0x55555555 的二进制形式是：01010101 01010101 01010101 01010101
            //用一个 8 位数字演示:假设v = 10110110,里面有 5 个 1，最终目标就是得到 5
            //把原始数据每两位分组：原始数据：10 | 11 | 01 | 10
            //                   1的数量： 1 |  2 |  1 |  1
            //                    二进制： 01 | 10 | 01 | 01
            v = v - ((v >> 1) & 0x55555555);//执行第一行后：v = 01100101
            //0x33333333 = 00110011 00110011 00110011 00110011
            v = (v & 0x33333333) + ((v >> 2) & 0x33333333);//执行第二行后：v = 00110010。现在每 4 位保存原始 4 个二进制位中 1 的数量。
            //继续使用v = 0011 0010
            //v >> 4  = 0000 0011
            //相加  = 0011 0101
            //& 0x0F  = 0000 0101=5
            //乘以 0x01010101 是一种并行累加技巧，它会让最高字节得到四个字节统计出的1的总数
            //>> 24右移取出总数
