...
namespace ORB_SLAM3 {
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
            ...
            if((sensor_ == System::MONOCULAR || sensor_ == System::IMU_MONOCULAR) && vPinHoleDistorsion1_.size() != 0){
                bNeedToUndistort_ = true;//需要消畸变
            }
        }
        //下面单目不看
        else if(cameraModel == "Rectified"){//已较正
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){//如果不需要双目矫正
                //Update calibration
                float scaleRowFactor = (float)newImSize_.height / (float)originalImSize_.height;//高度缩放倍数
                calibration1_->setParameter(calibration1_->getParameter(1) * scaleRowFactor, 1);//fy缩小
                calibration1_->setParameter(calibration1_->getParameter(3) * scaleRowFactor, 3);//cy缩小
    ...
    void Settings::readORB(cv::FileStorage &fSettings) {
        ...
        nFeatures_ = readParameter<int>(fSettings,"ORBextractor.nFeatures",found);//每一帧提取多少特征点
        scaleFactor_ = readParameter<float>(fSettings,"ORBextractor.scaleFactor",found);//金字塔缩放因子
        nLevels_ = readParameter<int>(fSettings,"ORBextractor.nLevels",found);//金字塔层数
        initThFAST_ = readParameter<int>(fSettings,"ORBextractor.iniThFAST",found);//初始检测阈值
        minThFAST_ = readParameter<int>(fSettings,"ORBextractor.minThFAST",found);//FAST太少时低阈值
    }
