...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mbShutdownArtifactsSaved(false)
{
    ...
    cout << "Seq. Name: " << strSequence << endl;//数据序列名称
    //在当前线程（主）中同步创建一个 Tracking 对象。(this:当前 System 对象地址，使 Tracking 能请求重置等系统操作,)
    ...
    mptLocalMapping = new thread(&ORB_SLAM3::LocalMapping::Run,mpLocalMapper);//启动LocalMapping线程，mpLocalMapper：指定在哪个对象上调用这个成员函数。
    mpLocalMapper->mInitFr = initFr;//初始化帧编号
    if(settings_)
        mpLocalMapper->mThFarPoints = settings_->thFarPoints();//设置地图最远点阈值
    else
    ...
    // mSensor!=MONOCULAR：是否固定尺度  单目：false          activeLC：是否启用地点识别、回环检测和地图合并
...
void System::Shutdown()
{
    ...
    const thread::id thisThread = this_thread::get_id();
    if(mptLocalMapping && mptLocalMapping->joinable() && mptLocalMapping->get_id()!=thisThread)
        mptLocalMapping->join();
    if(mptLoopClosing && mptLoopClosing->joinable() && mptLoopClosing->get_id()!=thisThread)
        mptLoopClosing->join();

    mpLoopCloser->WaitForGlobalBundleAdjustment();

    // Viewer::Run can call Shutdown from its own Pangolin menu. In that case
    // only the outer application call joins the viewer thread.
    if(mptViewer && mptViewer->joinable() && mptViewer->get_id()!=thisThread)
        mptViewer->join();

    if(!mbShutdownArtifactsSaved && !mStrSaveAtlasToFile.empty())
    {
        Verbose::PrintMess("Atlas saving to file " + mStrSaveAtlasToFile, Verbose::VERBOSITY_NORMAL);
        SaveAtlas(FileType::BINARY_FILE);
        mbShutdownArtifactsSaved = true;
    }
...
std::size_t System::SaveMapPointsPLY(const string &filename)
{
    vector<MapPoint*> vpMapPoints = mpAtlas->GetAllMapPoints();
    vector<Eigen::Vector3f> validPoints;
    validPoints.reserve(vpMapPoints.size());

    for(MapPoint* pMapPoint : vpMapPoints)
    {
        if(!pMapPoint || pMapPoint->isBad())
            continue;

        const Eigen::Vector3f position = pMapPoint->GetWorldPos();
        if(!std::isfinite(position.x()) || !std::isfinite(position.y()) ||
           !std::isfinite(position.z()))
            continue;

        validPoints.push_back(position);
    }

    ofstream cloud(filename.c_str(), ios::out | ios::trunc);
    if(!cloud.is_open())
        throw runtime_error("cannot open point-cloud output: " + filename);

    cloud << "ply\n"
          << "format ascii 1.0\n"
          << "comment ORB-SLAM3 sparse MapPoints from the active map\n"
          << "element vertex " << validPoints.size() << "\n"
          << "property float x\n"
          << "property float y\n"
          << "property float z\n"
          << "end_header\n";
    cloud << setprecision(9);
    for(const Eigen::Vector3f& position : validPoints)
        cloud << position.x() << ' ' << position.y() << ' ' << position.z() << '\n';

    cloud.close();
    if(!cloud)
        throw runtime_error("failed while writing point-cloud output: " + filename);

    cout << "Saved " << validPoints.size() << " sparse map points to "
         << filename << endl;
