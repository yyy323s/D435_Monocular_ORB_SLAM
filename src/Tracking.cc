...
{
    mImGray = im;
    if(mImGray.channels()==3)//如果是三通道彩色图转为灰度图
    {
        if(mbRGB)
            cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY);
        else
    ...
    if (mSensor == System::MONOCULAR)//如果传感器为纯单目（0）
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)//（未初始化||尚无图像||ID差小于mMaxFrames）
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);//（灰度图，时间戳，ORB提取器，ORB词典，相机内参，畸变参数，baseline*fx，远点阈值）
        else
    ...
    else if(mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);////（灰度图，时间戳，ORB提取器，ORB词典，相机内参，畸变参数，baseline*fx，远点阈值，上一帧地址，解引用IMU标定指针）
        }
        else
...
#ifdef REGISTER_TIMES
    vdORBExtract_ms.push_back(mCurrentFrame.mTimeORB_Ext);//本帧提取ORB耗时
#endif
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    pKFini->ComputeBoW();//把两个关键帧的ORB描述子转换成：BoW视觉单词权重；特征到词典节点的索引
    pKFcur->ComputeBoW();
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)//遍历初始化匹配
    {
        if(mvIniMatches[i]<0)
            continue;
        ...
        pKFini->AddMapPoint(pMP,i);//在KeyFrame的i号特征点上保存MapPoint
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);

        pMP->AddObservation(pKFini,i);//在MapPoint的i号三维点上保存KeyFrame
        pMP->AddObservation(pKFcur,mvIniMatches[i]);//第二幅图也绑定

        pMP->ComputeDistinctiveDescriptors();//计算地图点代表描述子，同一个地图点在不同关键帧中可能有不同ORB描述子。
        pMP->UpdateNormalAndDepth();//更新地图点方向和距离范围
    ...
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);//计算场景中值深度，解决单目尺度不确定性
    float invMedianDepth;//整个地图统一使用的缩放比例
    if(mSensor == System::IMU_MONOCULAR)
    ...
    // 缩放当前关键帧的基线
    Sophus::SE3f Tc2w = pKFcur->GetPose();//ORB-SLAM的 GetPose() 返回的是Tcw
    ...
    for(size_t iMP=0; iMP<vpAllMapPoints.size(); iMP++)
    {
        if(vpAllMapPoints[iMP])
        {
            ...
            pMP->SetWorldPos(pMP->GetWorldPos()*invMedianDepth);//缩放
