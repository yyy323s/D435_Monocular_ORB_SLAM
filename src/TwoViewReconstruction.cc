...
namespace ORB_SLAM3
{
    ...
    bool TwoViewReconstruction::Reconstruct(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2, const vector<int> &vMatches12,
                                             Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)//vP3D[i1]：第一帧第 i1 个特征对应的三维点。vbTriangulated[i1]：该三维点是否成功、可靠地完成三角化。
    {
        mvKeys1.clear();
        mvKeys2.clear();
        ...
        {
            if(vMatches12[i]>=0)//如果图二上有对应的特征点
            {
                mvMatches12.push_back(make_pair(i,vMatches12[i]));//i是图一特征点下标，vMatches12[i]是在图二与之对应的特征下标
                mvbMatched1[i]=true;//标记第一帧哪些特征有匹配
            }
            else
                mvbMatched1[i]=false;
        }

        const int N = mvMatches12.size();//匹配对的总数N

        // Indices for minimum set selection
        vector<size_t> vAllIndices;//保存全部可抽取的内部匹配编号
        vAllIndices.reserve(N);
        vector<size_t> vAvailableIndices;//当前一次 RANSAC 迭代中还没有被抽到的编号。
        ...
        for(int it=0; it<mMaxIterations; it++)
        {
            ...
            for(size_t j=0; j<8; j++)
            {
                ...
                vAvailableIndices[randi] = vAvailableIndices.back();//把最后一个覆盖到randi，保证不会重复抽到同一个点
                vAvailableIndices.pop_back();//把最后一个删掉
            }
        }

        // Launch threads to compute in parallel a fundamental matrix and a homography
        //vbMatchesInliersH[k]：内部第 k 对匹配是否为单应矩阵内点；
        //vbMatchesInliersF[k]：内部第 k 对匹配是否为基础矩阵内点。
        ...
        Eigen::Matrix3f H, F;
        //启动两个线程
        //&TwoViewReconstruction::FindHomography要执行的成员函数地址
        //this在当前TwoViewReconstruction对象上执行
    ...
    Eigen::Matrix3f TwoViewReconstruction::ComputeF21(const vector<cv::Point2f> &vP1,const vector<cv::Point2f> &vP2)
    {
        ...
        Eigen::JacobiSVD<Eigen::Matrix3f> svd2(Fpre, Eigen::ComputeFullU | Eigen::ComputeFullV);//F理论上要求秩为 2
    ...
    float TwoViewReconstruction::CheckHomography(const Eigen::Matrix3f &H21, const Eigen::Matrix3f &H12, vector<bool> &vbMatchesInliers, float sigma)
    {
        ...
        for(int i=0; i<N; i++)
        {
            ...
            // x2in1 = H12*x2  :image2上的点通过H12投影到image1上

            const float w2in1inv = 1.0/(h31inv*u2+h32inv*v2+h33inv);
            const float u2in1 = (h11inv*u2+h12inv*v2+h13inv)*w2in1inv;
            const float v2in1 = (h21inv*u2+h22inv*v2+h23inv)*w2in1inv;

            const float squareDist1 = (u1-u2in1)*(u1-u2in1)+(v1-v2in1)*(v1-v2in1);//重投影误差的平方

            const float chiSquare1 = squareDist1*invSigmaSquare;//噪声标准差倍数的平方
    ...
    float TwoViewReconstruction::CheckFundamental(const Eigen::Matrix3f &F21, vector<bool> &vbMatchesInliers, float sigma)
    {
        ...
        const float th = 3.841;//3.841 是卡方分布自由度为 1、置信度约为 95%时的阈值。判断是否为内点（及格线）
        const float thScore = 5.991;//5.991 是自由度为 2、置信度约为 95%时的常见卡方阈值。每一次加分的上限
        ...
        for(int i=0; i<N; i++)
        {
            ...
            const float squareDist1 = num2*num2/(a2*a2+b2*b2);//d～N（0,sigma^2）,d/sigma~N(0,sigma)：表示这个这个误差（d）是几倍的噪声标准差,sigma(观测噪声的标准差)

            const float chiSquare1 = squareDist1*invSigmaSquare;//距离的平方(d^2)/Sigma^2:卡方分布（上边那个的平方）    chiSquare = 错误大小，越小越好
            ...
            const float squareDist2 = num1*num1/(a1*a1+b1*b1);//点到直线距离的平方
                                             ...
                                             Sophus::SE3f &T21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
    {
        ...
        // 我们使用Faugeras等人提出的方法恢复了8个运动假设。
        // 分段平面环境中的运动与运动结构。国际模式识别与人工智能杂志，1988
        //
        Eigen::Matrix3f invK = K.inverse();
        Eigen::Matrix3f A = invK * H21 * K;//H21 = K*（R+tnT/d）invK,左乘invk右乘K得到中间部分
        ...
        float s = U.determinant() * Vt.determinant();//SVD得到的UV可能行列式为-1，s用来修正UV

        float d1 = w(0);
        float d2 = w(1);
        float d3 = w(2);

        if(d1/d2<1.00001 || d2/d3<1.00001)//相邻奇异值是否过于接近，因为纯旋转时A～R，若此时三个奇异值相等，没有平移基线，无法稳定三角化出三维地图
        {
            return false;//要保证d1>d2>d3
        }
    ...
    int TwoViewReconstruction::CheckRT(const Eigen::Matrix3f &R, const Eigen::Vector3f &t, const vector<cv::KeyPoint> &vKeys1, const vector<cv::KeyPoint> &vKeys2,
                                       ...
                                       const Eigen::Matrix3f &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)//th2:允许的最大平方重投影误差阈值。//vbGood:哪些三维点可以真正用于初始化地图。
    {                                                                                                                                        //vP3D:三角化出的三维点，按第一帧特征索引保存//parallax这一组重建结果的代表性视差角，单位为度
        ...
        vbGood = vector<bool>(vKeys1.size(),false);//初始化数组
        ...
        P1.setZero();
        P1.block<3,3>(0,0) = K;//R=I，t=0

        Eigen::Vector3f O1;//第一台相机中心
        O1.setZero();
        ...
        Eigen::Vector3f O2 = -R.transpose() * t;//第二台相机中心
        ...
        for(size_t i=0, iend=vMatches12.size();i<iend;i++)
        {
            ...
            Eigen::Vector3f p3dC2 = R * p3dC1 + t;//坐标系转换
            ...
            vP3D[vMatches12[i].first] = cv::Point3f(p3dC1(0), p3dC1(1), p3dC1(2));//按照第一帧特征索引保存三维点。
            nGood++;

            if(cosParallax<0.99998)//cos(theta)=0.99998,theta=0.36度，cos<0.99998代表theta>0.36
                vbGood[vMatches12[i].first]=true;
        }

        if(nGood>0)
        {
            sort(vCosParallax.begin(),vCosParallax.end());//cos从小到大排序，对应theta从大到小

            size_t idx = min(50,int(vCosParallax.size()-1));//theta从大到小，如果总点数大于50，这里min取前面50,保证前面有50个theta大于这个度数
            parallax = acos(vCosParallax[idx])*180/CV_PI;//求度数。如果总点数小于50，那么取后面这个最后的，保证它前面的的的度数都大于最后一个
        }
        else
            parallax=0;
